...
import { enterLine, exitLine, StyleAnimation, emptyStyle, fadeInFocus, fadeOutFocus } from './animation'
...
export function LineList({ stepPair, t, tokens, types, dimensions, unfocusedStyle, maxLineCount, showNumbers }: LineListProps) {
  const lines = React.useMemo(() => {
    ...
    const fadeInLines = linesPair
      .map((linePair, lineKey: number) => {
        ...
        const isFadeIn = !prev || (!prevFocus && nextFocus) || (nextFocus && Array.isArray(prevFocus))
    ...
    const fadeOutLines = linesPair
      .map((linePair, lineKey: number) => {
        ...
        const [prevFocus, nextFocus] = linePair.select((l) => l.focus).spread()
        const isFadeOut = !next || (!nextFocus && prevFocus) || (prevFocus && Array.isArray(nextFocus))
    ...
    return linesPair.map((lineTuple, lineKey) => {
      ...
      const areTokensAnimated = !isStatic && (Array.isArray(prevFocus) || Array.isArray(nextFocus))
      ...
      const lineNumber = anyLine ? (pad + anyLine.lineNumber).slice(-pad.length) : ''
      const lineNumberElement = showNumbers && <span className={'token-line-number'}>{lineNumber + ' '}</span>
      ...
      const lineElement = isStatic && (
        ...
        >
          {lineNumberElement}
          <div style={{ display: 'inline-block' }} className={`cs-line cs-line-${lineKey}`}>
            {tokenElements}
          </div>
        </div>
      )
      ...
      if (!isStatic) {
        if (!prevLine) {
          ...
          getLineStyle = exitLine(fromOpacity, toOpacity, fadeOutIndex, fadeOutLines.length, lineHeight)
        } else if (!prevFocus && nextFocus && !Array.isArray(nextFocus)) {
          const fadeInIndex = fadeInLines.indexOf(lineKey)
          getLineStyle = fadeInFocus(offOpacity, 1, fadeInIndex, fadeInLines.length)
        } else if (prevFocus && !nextFocus && !Array.isArray(prevFocus)) {
          const fadeOutIndex = fadeOutLines.indexOf(lineKey)
          getLineStyle = fadeOutFocus(1, offOpacity, fadeOutIndex, fadeOutLines.length)
        }
      }

      let getTokenStyle: (t: number, i: number) => CSSProperties = emptyStyle
      if (!areTokensStatic) {
        const fromFocus = tokens[lineKey].map((_, tokeni) => (Array.isArray(prevFocus) ? prevFocus.includes(tokeni) : prevFocus))
        const toFocus = tokens[lineKey].map((_, tokeni) => (Array.isArray(nextFocus) ? nextFocus.includes(tokeni) : nextFocus))
        const fadeInIndex = fadeInLines.indexOf(lineKey)
        const fadeOutIndex = fadeOutLines.indexOf(lineKey)
        getTokenStyle = (t, i) => {
          ...
          const animation =
            fromOpacity < toOpacity
              ? fadeInFocus(fromOpacity, toOpacity, fadeInIndex, fadeInLines.length)
              : fadeOutFocus(fromOpacity, toOpacity, fadeOutIndex, fadeOutLines.length)
          return animation(t)
        }
      }
  ...
  return (
    <React.Fragment>
      {lines.map(
        ({ lineElement, lineKey, tokenElements, getLineStyle, getTokenStyle, lineNumberElement }) =>
          lineElement || (
            <div style={{ overflow: 'hidden', ...getLineStyle(t) }} key={lineKey}>
              <div style={{ display: 'inline-block' }} className={`cs-line cs-line-${lineKey}`}>
                {lineNumberElement}
                {tokenElements ||
                  tokens[lineKey].map((token, tokeni) => (
                    <span className={'token-' + types[lineKey][tokeni]} style={getTokenStyle!(t, tokeni)} key={tokeni}>
                      {token}
                    </span>
                  ))}
              </div>
