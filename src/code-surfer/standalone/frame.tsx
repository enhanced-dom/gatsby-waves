...
import { Styled, useUnfocusedStyle } from '../themes'
import { LineList } from './lines'
import { fadeOutIn, halfFadeIn, halfFadeOut, scrollToFocus, scaleToFocus } from './animation'
...
function CodeSurferContainer({ progress, dimensions, steps, tokens, types, maxLineCount, showNumbers }: ContainerProps) {
  ...
  const titlePair = React.useMemo(() => stepPair.select((step) => step.title), [stepPair])
  const subtitlePair = React.useMemo(() => stepPair.select((step) => step.subtitle), [stepPair])
...
function CodeSurferContent({
  ...
  const scaleAnimation = React.useMemo(() => scaleToFocus(stepPair, dimensions), [stepPair, dimensions])

  const scrollAnimation = React.useMemo(() => scrollToFocus(stepPair, dimensions), [stepPair, dimensions])

  const scale = scaleAnimation(t)
  const scrollTop = scrollAnimation(t)
  const verticalOrigin = dimensions ? dimensions.containerHeight / 2 + scrollTop : 0
...
function Title({ textPair, t }: CaptionProps) {
  ...
  const backgroundStyle = prev && next ? undefined : !prev ? halfFadeIn()(t) : halfFadeOut()(t)
