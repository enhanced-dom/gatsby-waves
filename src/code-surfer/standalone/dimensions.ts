...
function useDimensions<T extends HTMLElement | null>(ref: React.MutableRefObject<T>, steps: Step[]): DimensionsResult {
  ...
  React.useLayoutEffect(() => {
    ...
    const containers = ref.current.querySelectorAll('.cs-container') as NodeListOf<HTMLElement>

    const stepsDimensions = Array.from(containers).map((container, i) => getStepDimensions(container, steps[i]))

    const containerHeight = Math.max(...stepsDimensions.map((d) => d.containerHeight))

    const containerWidth = Math.max(...stepsDimensions.map((d) => d.containerWidth))
...
function getStepDimensions(container: HTMLElement, step: Step) {
  ...
  const heightOverflow = containerParent.scrollHeight - containerParent.clientHeight
